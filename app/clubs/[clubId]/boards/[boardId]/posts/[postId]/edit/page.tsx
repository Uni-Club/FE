  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        router.push(`/clubs/${clubId}/boards/${boardId}/posts/${postId}`);
      } else {
  ...
  return (
    <main className="pt-28 pb-20 px-4 sm:px-6 lg:px-8 min-h-screen bg-slate-50">
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          <Link href={`/clubs/${clubId}/boards/${boardId}/posts/${postId}`} className="text-indigo-600 hover:underline mb-2 inline-block">
            ← 돌아가기
          </Link>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 border border-slate-200">
          ...
          <div className="flex gap-4 mt-8">
            ...
            <Link href={`/clubs/${clubId}/boards/${boardId}/posts/${postId}`} className="flex-1">
              <Button
                type="button"
                variant="secondary"
